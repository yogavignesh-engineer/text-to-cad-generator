...
function validateGeometry(parsed) {
  ...
  if (!parsed) {
    errors.push("Could not parse prompt");
  ...
  Object.entries(dimensions).forEach(([key, value]) => {
    if (value < 0.1) errors.push(`${key} too small (min 0.1mm)`);
    if (value > 10000) errors.push(`${key} too large (max 10000mm)`);
    if (value > 5000) warnings.push(`Large ${key} (${value}mm) may slow rendering`);
  });
  
  if (shape === 'gear') {
    if (dimensions.teeth < 6) errors.push("Gear needs at least 6 teeth");
    if (dimensions.teeth > 200) warnings.push("Large tooth count may slow rendering");
    if (!features?.threaded) suggestions.push("💡 Add center hole? Try: 'with M8 center hole'");
  }
  
  if (shape === 'box') {
    if (features?.fillet) {
      const maxFillet = Math.min(dimensions.length, dimensions.width, dimensions.height) / 2;
    ...
    if (!features?.chamfer && dimensions.height < 15) {
