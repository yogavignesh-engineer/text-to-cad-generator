...
try:
    ...
    for edge in shape.Edges:
        if edge.Length > 1.5 * 4:
            edges_to_chamfer.append(edge)

    if len(edges_to_chamfer) > 0:
        print(f"[FREECAD] Chamfering {len(edges_to_chamfer)} edges with size=1.5mm")
        chamfered_shape = shape.makeChamfer(1.5, edges_to_chamfer)
        ...
        base = chamfer_0
        doc.recompute()
        print(f"[FREECAD] Chamfer applied (size=1.5mm on {len(edges_to_chamfer)} edges)")
    else:
        print(f"[FREECAD] No suitable edges for 1.5mm chamfer")
...
# EXPORT TO STL
import Mesh
output_path = r"C:\Users\_YOGA_VIGNESH_\Videos\text to cad\outputs\1766474472.stl"
