...
try:
    ...
    if len(edges_to_chamfer) > 0:
        print(f"[FREECAD] Chamfering {len(edges_to_chamfer)} edges with size=2.0mm")
        
        # Create chamfered shape
        chamfered_shape = shape.makeChamfer(2.0, edges_to_chamfer)
        ...
        print(f"[FREECAD] ✓ Chamfer applied: 2.0mm on {len(edges_to_chamfer)} edges")
    else:
        print(f"[FREECAD] ⚠ No suitable edges for 2.0mm chamfer")
...
import Mesh
import Part
output_path_stl = r"C:\Users\_YOGA_VIGNESH_\Videos\text to cad\outputs\output_1766379736.stl"
output_path_step = r"C:\Users\_YOGA_VIGNESH_\Videos\text to cad\outputs\output_1766379736.step"
