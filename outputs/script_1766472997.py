...
try:
    ...
    for edge in shape.Edges:
        if edge.Length > 5.0 * 4:
            edges_to_chamfer.append(edge)

    if len(edges_to_chamfer) > 0:
        print(f"[FREECAD] Chamfering {len(edges_to_chamfer)} edges with size=5.0mm")
        chamfered_shape = shape.makeChamfer(5.0, edges_to_chamfer)
        ...
        base = chamfer_0
        doc.recompute()
        print(f"[FREECAD] Chamfer applied (size=5.0mm on {len(edges_to_chamfer)} edges)")
    else:
        print(f"[FREECAD] No suitable edges for 5.0mm chamfer")
...
# EXPORT TO STL
import Mesh
output_path = r"C:\Users\_YOGA_VIGNESH_\Videos\text to cad\outputs\1766472997.stl"
